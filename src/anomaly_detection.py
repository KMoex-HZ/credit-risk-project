...
def fit_anomaly_detector(df, contamination=0.02, random_state=42):
    """Fit Isolation Forest, return fitted model + scaler (buat dipakai ulang ke data test)"""
...
def apply_anomaly_detector(df, iso_forest, scaler):
    """Apply fitted detector ke data (train atau test), return df dengan kolom tambahan"""
    df = df.copy()
    X = df[ISO_FEATURES].copy()
